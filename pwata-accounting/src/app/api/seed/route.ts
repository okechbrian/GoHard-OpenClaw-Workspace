...
export async function POST() {
  try {
    ...
    const custStmt = sqlite.prepare("INSERT INTO customers (id, name, phone, email) VALUES (?, ?, ?, ?)");
    for (const c of customers) {
      const id = generateId();
      customerIds.push(id);
      custStmt.run(id, c.name, c.phone, c.email || null);
    }

    // Seed sales
    const saleStmt = sqlite.prepare("INSERT INTO sales (id, customer_id, description, amount, payment_method, payment_status, sale_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
    const salesData = [
    ...
    for (const s of salesData) {
      saleStmt.run(generateId(), customerIds[s.cust], s.desc, s.amount, s.method, "paid", s.date);
    }

    // Seed expenses
    const expStmt = sqlite.prepare("INSERT INTO expenses (id, category, description, amount, payment_method, expense_date) VALUES (?, ?, ?, ?, ?, ?)");
    const expData = [
    ...
    for (const e of expData) {
      expStmt.run(generateId(), e.cat, e.desc, e.amount, e.method, e.date);
    }
